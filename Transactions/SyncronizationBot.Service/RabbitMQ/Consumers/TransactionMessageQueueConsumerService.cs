...
{
    public class TransactionMessageQueueConsumerService : BaseSendTelegramMessageConsumer
    {
        ...
        public TransactionMessageQueueConsumerService(IServiceProvider serviceProvider, IOptions<SyncronizationBotConfig> configs, IOptions<LogMessageQueueConfig> configuration) : base(serviceProvider, configuration.Value, configs)
        {
            this.Mediator = null!;
        ...
        public override async Task HandlerAsync(IServiceScope scope, string? message, CancellationToken stoppingToken)
        {
            ...
            if (@event != null)
            {
                ...
                {
                    var channel = await this.TelegramChannelService.FindFirstOrDefaultAsync(x => x.ID == TelegramChannelId);
                    if (channel != null)
                    {
